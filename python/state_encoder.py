...
import numpy as np
from catan_engine import GameState, PieceType, HarborType, Resource, GamePhase, TurnPhase
...
class StateEncoder:
    ...
    def __init__(self):
        ...
        # Feature dimensions (static expectations, used only for documentation)
        # Actual total_features is computed dynamically below to stay in sync
        # with encode_state() so shape mismatches cannot occur.
        ...
        self.global_features = 20  # current player, phase, longest road/army owners, num players

        # Dynamically compute total feature size from a dummy state to avoid
        # any drift between declared size and actual encoded vector length.
        dummy_state = GameState.create_new_game(4, 0)
        dummy_features = self.encode_state(dummy_state, 0)
        self.total_features = int(dummy_features.shape[0])
    ...
    def _encode_global(self, state: GameState, perspective: int) -> list:
        ...
        features.append(1.0 if state.game_phase == GamePhase.Setup else 0.0)
        features.append(1.0 if state.game_phase == GamePhase.MainGame else 0.0)
        
        # Turn phase (one-hot: RollDice, Discard, Robber, Trading)
        turn_phase_one_hot = [0.0] * 4
        if state.turn_phase == TurnPhase.RollDice:
            turn_phase_one_hot[0] = 1.0
        elif state.turn_phase == TurnPhase.Discard:
            turn_phase_one_hot[1] = 1.0
        elif state.turn_phase == TurnPhase.Robber:
            turn_phase_one_hot[2] = 1.0
        elif state.turn_phase == TurnPhase.Trading:
            turn_phase_one_hot[3] = 1.0
